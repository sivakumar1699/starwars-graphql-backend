...
exports.validateRelayIdDirective = validateRelayIdDirective;
...
function validateRelayIdDirective(context) {
    ...
    return {
        FieldDefinition(fieldDefinitionNode, _key, _parent, path, ancestors) {
            if (!fieldDefinitionNode.directives?.find((directive) => directive.name.value === directives_1.relayIdDirective.name)) {
                return;
            }
            ...
            const { isValid, errorMsg } = (0, document_validation_error_1.assertValid)(() => {
                if (!isValidLocation) {
                    throw new document_validation_error_1.DocumentValidationError(`Directive "${directives_1.relayIdDirective.name}" requires in a type with "@node"`, []);
                }
            });
            ...
            if (!isValid) {
                context.reportError((0, document_validation_error_1.createGraphQLError)({
                    ...
                    path: [...pathToNode[0], `@${directives_1.relayIdDirective.name}`],
                    errorMsg,
                }));
            }
        },
